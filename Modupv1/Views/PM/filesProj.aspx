<%@ Page Title="" Language="C#" MasterPageFile="~/Views/PM/PMWorkroom.Master" Inherits="System.Web.Mvc.ViewPage<Modupv1.Models.FileModel>" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $('#file').change(function () {
                ...
                if (f.files.length > 0) {
                    ...
                    if (window.FormData !== undefined) {
                        ...
                        data.append("wr", '<%:ViewBag.wroomID%>');
                        console.log("2");
                        $.ajax({
                            url: '<%:Url.Action("filesProj","PM")%>',
                            method: 'POST',
                            data: data,
                            processData: false,
                            contentType: false,
                            ...
                            success: function (resp) {
                                $.each(resp, function (index, row) {
                                    $.each(row, function (key, value) {
                                        ...
                                        var $tr = $('<tr>').append(
                                           $('<td>').html("<a href='http://localhost:49699/PM/DownloadWR/?id=" +<%:ViewBag.wroomID%> +"&filename=" + value.filename + "'>" + value.filename + "</a>"),
            ...
            $('.btn.btn-danger').unbind().click(function () {
                ...
                var wrID = '<%:ViewBag.wroomID%>';
                $(this).parents('tr').attr('id', wrID);
                $.ajax({
                    url: '<%:Url.Action("remFileProjWR","PM")%>',
                    method: 'POST',
